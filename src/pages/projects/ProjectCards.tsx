...
const ProjectCards: React.FC = () => {
  ...
  const displayProjects = projectData.map((item, i) => {
    ...
    const displayTools = tools.map((tool, i) => (
    ...
    return (
      ...
      >
        <motion.div
          className="w-full aspect-[1.2/1] flex flex-col 
        ...
        <motion.div
          className="grid grid-cols-2 place-items-center bg-title w-full 
        ...
        >
          ...
          <Link
            to={`/projects/${item.pageRoute}`}
            className="row-start-3 text-white ml-8 md:ml-12 self-center 
          ...
          <div className="col-start-2 row-start-3 flex items-center gap-6 ml-auto mr-8 md:mr-12">
            ...
            >
              <img
                src={LinkIcon}
                alt="link logo"
                className="w-4 lg-2:w-[1.4rem]"
              />
            </motion.a>
          </div>

          <ul
            className="row-start-4 col-start-1 col-span-full flex flex-wrap gap-8 justify-center
